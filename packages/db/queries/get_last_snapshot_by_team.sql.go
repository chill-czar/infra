...
// versions:
//   sqlc v1.29.0
// source: get_last_snapshot_by_team.sql
...
const getLastSnapshotByTeam = `-- name: GetLastSnapshotByTeam :one
...
type GetLastSnapshotByTeamParams struct {
	SandboxID string
	TeamID    uuid.UUID
}

type GetLastSnapshotByTeamRow struct {
	Aliases  []string
	Names    []string
	Snapshot Snapshot
	EnvBuild EnvBuild
...
func (q *Queries) GetLastSnapshotByTeam(ctx context.Context, arg GetLastSnapshotByTeamParams) (GetLastSnapshotByTeamRow, error) {
	row := q.db.QueryRow(ctx, getLastSnapshotByTeam, arg.SandboxID, arg.TeamID)
	var i GetLastSnapshotByTeamRow
	err := row.Scan(
		&i.Aliases,
		&i.Names,
